...
const dbPath = process.env.VERCEL ? '/tmp/portfolio.db' : path.join(__dirname, 'portfolio.db');
    ...
    const db = new sqlite3.Database(dbPath, (err) => {
      ...
      db.serialize(() => {
        ...
        db.run(`CREATE TABLE IF NOT EXISTS works (
          ...
          if (err) {
            console.error('Error creating works table:', err);
            reject(err);
            return;
          }
        });
...
async function insertDefaultData(db) {
  return new Promise((resolve, reject) => {
    ...
    db.get("SELECT COUNT(*) as count FROM homepage", (err, row) => {
      if (err) {
        console.error('Error checking homepage data:', err);
        reject(err);
        return;
      }
      ...
      if (row.count === 0) {
        ...
        'SERVICES', 'HIRE ME')`, (err) => {
          if (err) {
            console.error('Error inserting homepage data:', err);
            reject(err);
            return;
          }
        });
      ...
      db.get("SELECT COUNT(*) as count FROM about", (err, row) => {
        if (err) {
          console.error('Error checking about data:', err);
          reject(err);
          return;
        }
        ...
        if (row.count === 0) {
          ...
          'CAREER', 'TECH STACK', 'SERVICES', 'HIRE ME')`, (err) => {
            if (err) {
        ...
        db.get("SELECT COUNT(*) as count FROM contact", (err, row) => {
          ...
          if (row.count === 0) {
            ...
            VALUES (1, 'Lets''s build something great togather', '+91 7907348596', 'mithunmacsafe@gmail.com')`, (err) => {
              if (err) {
